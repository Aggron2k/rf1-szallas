    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(request ->
                        request.requestMatchers("/", "/register", "/login", "/registerRequest", "/saveUser", "/css/**", "/images/**", "/search","/szallasAdmin","/deleteSzallas/**","/modifyAccommodation").permitAll().anyRequest().authenticated())
